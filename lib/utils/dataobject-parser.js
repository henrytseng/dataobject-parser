...
DataObjectParser.prototype.set = function($path, $value) {
  ...
  var _self = this;
  var re = /[\$\w-|]+|(\[\d+\])|(\[\])|(["'](.*?)['"])/g;
  // parse $path on dots. brackets are left intact because we use them later to determine if the key is for an array or object
  ...
  while(pathList.length > 0) {
    var nextKey = pathList.shift();
    // The key is for an array if it's in brackets
    var isArray = nextKey.startsWith("[")
    // Strip off the brackets now that we've used them to determine if it's an array or not.
    // We also strip off any quotes.
    // [] is a special case that we leave intact because it's used to append to an array
    parentKey = nextKey === "[]" ? nextKey : nextKey.replace(/[\[\]"']/g, '');
